		   ...
		   if [[ $count%2 -ne 0 ]]; then
		       ...
		       R2_unpair=${sample_name}_R2_unpair${extension}
                       echo -e "\n${yellow}[Running trimmomatic for sample] ${sample_name} at `whoami`${reset}\n"
                         date && time java -jar \
            /home/tiagobelintani/programas/Trimmomatic-0.39/trimmomatic-0.39.jar \
            PE \
            -threads 40 \
            -phred33 $R1 $R2 $R1_pair $R1_unpair $R2_pair $R2_unpair \
            HEADCROP:7 \
            ILLUMINACLIP:/home/tiagobelintani/programas/Trimmomatic-0.39/adapters/TruSeq3-PE.fa:2:30:10:2:keepBothReads \
            LEADING:20 \
            TRAILING:20 \
            SLIDINGWINDOW:05:20 \
            MINLEN:50 
